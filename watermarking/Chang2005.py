...
from numpy import abs, angle, empty, empty_like, exp
...
class Watermarker:
    ...
    def __init__(self, scale_factor=0.012) -> None:
    ...
    def add_watermark(self, host_image, watermark_image):
        ...
        for i in range(0, watermark_image.shape[0]):
            for j in range(0, watermark_image.shape[1]):
                U, s, Vh = svd(
                    host_image[
                        i * self.R1 : (i + 1) * self.R1, j * self.R2 : (j + 1) * self.R2
                    ],
                    full_matrices=False,
                )
    ...
    def extract_watermark(self, watermarked_image):
        ...
        watermark = empty(
            shape=(watermarked_image.shape[0] // R, watermarked_image.shape[1] // R)
        )
